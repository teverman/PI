...
WebServer::WebServer(TorMgr *tor_mgr, int port)
    : tor_mgr(tor_mgr), port(port) { }
...
WebServer::query_counter(const std::string &counter_name, size_t index,
  ...
  return tor_mgr->query_counter(counter_name, index, packets, bytes);
}

int
WebServer::update_json_config(const std::string &config_buffer) {
  return tor_mgr->update_config(config_buffer);
}
